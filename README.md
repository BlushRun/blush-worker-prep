...
- prepare one capability: `uv run worker-prep validate --repo submodules/workflow-flux2-klein-9b` then `uv run worker-prep hydrate-build --repo submodules/workflow-flux2-klein-9b --force`
- start locally: `cd submodules/workflow-flux2-klein-9b` then `docker compose --env-file .env.local.example -f docker-compose.yml up --build -d`
- local smoke: `uv run worker-prep smoke-local --repo submodules/workflow-flux2-klein-9b req/workflow-flux2-klein-9b-t2i.api.json`
